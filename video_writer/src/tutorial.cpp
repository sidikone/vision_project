...
void TutoLib::video_reader_tutorial(){
    ...
    video_1.showVideo(*video_name_ptr, 33, "gray", true);

}

void TutoLib::video_writer_tutorial(){
    ...
    // Read Video from path
    cout << data_set.video_path << endl;
    Recording rec_1(data_set.video_path);

    int codec = VideoWriter::fourcc('M', 'J', 'P', 'G');  // select desired codec (must be available at runtime)
    double fps = 25.0;                          // framerate of the created video stream
    rec_1.SetParameters("./video.mjpg", codec, fps, 2, true);    

    rec_1.WriteVideo(*video_name_ptr, 33, "gray", true);
